...
BDPTR sub(BDPTR T,char s,int k)
{
	...
	if(s=='l'){
		T->key[i]=mx(T->ptr[i]);
		del(T->ptr[i],T->key[i],T);
	}
	else{
		T->key[i]=mn(T->ptr[i+1]);
		del(T->ptr[i+1],T->key[i],T);
	}
	return T;
}

void N_C(BDPTR &P,int i,BDPTR &root,int d,int x){
    BDPTR T=P->ptr[i],L=P->ptr[i+1];
    int j=0;
    while(j<T->cnt)
    {
        if(T->key[j]==x)
    ...
    while(j<L->cnt){
        ...
        T->key[T->cnt++]=L->key[j];
        if(j+1==L->cnt)
            T->ptr[T->cnt]=L->ptr[j+1];
        j++;
    }
}

void del(BDPTR &T,int k,BDPTR Parent)
{
	if(T!=NULL){
		int i=0;
